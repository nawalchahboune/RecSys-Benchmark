...
def main():
    ...
    if (
        ...
        checkpoints = [ckpt for ckpt in os.listdir(training_args.output_dir) if os.path.isdir(os.path.join(training_args.output_dir, ckpt))]
        if len(checkpoints) > 0: 
            most_recent_checkpoint = os.path.join(training_args.output_dir, sorted(checkpoints)[-1])
            logger.info(
                f"---- try resuming from checkpoint: {most_recent_checkpoint}... ----"
            )
        else: 
            raise ValueError(
    ...
    # trainer.train()
    trainer.train(resume_from_checkpoint=most_recent_checkpoint)
