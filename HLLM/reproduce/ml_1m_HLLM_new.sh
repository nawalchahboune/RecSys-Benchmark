#!/bin/bash
#SBATCH --job-name=hllm_ml-1m_train
#SBATCH --nodes=1            
...
#SBATCH --output=outputs/hllm_ml-1m_train_%j.out
#SBATCH --error=outputs/hllm_ml-1m_train_%j.err
...
python3 main.py \
  --config_file overall/LLM_deepspeed_ml.yaml HLLM/HLLM.yaml \
  --loss nce \
  --epochs 5 \
  --train_batch_size 32 \
  --MAX_TEXT_LENGTH 256 \
